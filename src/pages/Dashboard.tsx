...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Validate hospital ID is a valid UUID
    const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
    if (!uuidRegex.test(hospital.id)) {
      setIsLoading(false);
      setError('Invalid hospital ID format');
      return;
    }
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      // First check if we have access to the patients table
      ...
      if (accessError) {
        if (accessError.code === 'PGRST301') {
          throw new Error('You do not have permission to access patient data');
        }
        throw accessError;
      }
      ...
      if (consultationsError) {
        ...
        throw new Error(`Error fetching consultations: ${consultationsError.message}`);
      }
      ...
      if (labResultsError) {
        console.error('Lab results query error:', labResultsError);
        throw new Error(`Error fetching lab results: ${labResultsError.message}`);
      }
      ...
      if (pharmacyError) {
        console.error('Pharmacy query error:', pharmacyError);
        throw new Error(`Error fetching pharmacy orders: ${pharmacyError.message}`);
      }
      ...
      if (emergencyError) {
        console.error('Emergency cases query error:', emergencyError);
        throw new Error(`Error fetching emergency cases: ${emergencyError.message}`);
      }
      ...
      if (activePatientsError) {
        console.error('Active patients query error:', activePatientsError);
        throw new Error(`Error fetching active patients: ${activePatientsError.message}`);
      }
