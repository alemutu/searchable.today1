...
import { FileText, Plus, Trash2, User, Activity, AlertTriangle, Clock, FileBarChart2, Pill, AlertCircle, Search, Calendar, ChevronDown, ChevronUp } from 'lucide-react';
...
const ConsultationForm: React.FC = () => {
  ...
  const [medicationSearchResults, setMedicationSearchResults] = useState<string[]>([]);
  ...
  const commonMedications = [
    ...
    { name: 'Levothyroxine (Synthroid)', dosages: ['25mcg', '50mcg', '75mcg', '88mcg', '100mcg', '112mcg', '125mcg', '137mcg', '150mcg', '175mcg', '200mcg'] },
    { name: 'Albuterol (Ventolin)', dosages: ['90mcg/actuation', '108mcg/actuation'] },
    { name: 'Fluticasone (Flonase)', dosages: ['50mcg/spray'] },
    { name: 'Montelukast (Singulair)', dosages: ['4mg', '5mg', '10mg'] },
    { name: 'Sertraline (Zoloft)', dosages: ['25mg', '50mg', '100mg'] },
    { name: 'Escitalopram (Lexapro)', dosages: ['5mg', '10mg', '20mg'] },
    { name: 'Loratadine (Claritin)', dosages: ['10mg'] },
    { name: 'Cetirizine (Zyrtec)', dosages: ['5mg', '10mg'] },
    { name: 'Diphenhydramine (Benadryl)', dosages: ['25mg', '50mg'] },
    ...
    { name: 'Cyclobenzaprine', dosages: ['5mg', '10mg'] },
    { name: 'Meloxicam', dosages: ['7.5mg', '15mg'] },
    { name: 'Ciprofloxacin', dosages: ['250mg', '500mg', '750mg'] },
    { name: 'Metronidazole', dosages: ['250mg', '500mg'] },
    { name: 'Fluconazole', dosages: ['50mg', '100mg', '150mg', '200mg'] }
  ];
  ...
  useEffect(() => {
    ...
    setMedicationSearchResults(filteredMedications);
  ...
  const handleMedicationSelect = (medication: string, index: number) => {
    // Find the selected medication in the common medications list
  ...
  const handleAddCustomMedication = () => {
    if (customMedication.trim() === '' || selectedMedicationIndex === null) return;
    
    setValue(`prescriptions.${selectedMedicationIndex}.medication`, customMedication);
    setCustomMedication('');
  ...
  const handlePrintMedicalCertificate = () => {
    if (!medicalCertificateDetails || !patient) return;
    
    // Create a new window for the certificate
    const certificateWindow = window.open('', '_blank');
    if (!certificateWindow) {
      alert('Please allow popups to print the medical certificate');
      return;
    }
    ...
    const startDate = new Date(medicalCertificateDetails.startDate).toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'long',
      day: 'numeric'
    });
    
    const endDate = new Date(medicalCertificateDetails.endDate).toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'long',
      day: 'numeric'
    });
    ...
    const start = new Date(medicalCertificateDetails.startDate);
    const end = new Date(medicalCertificateDetails.endDate);
    const daysOfRest = Math.round((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24)) + 1;
    ...
    const certificateHTML = `
      ...
      <body>
        <div class="header">
          <div class="logo">HMS - Hospital Management System</div>
        ...
        <div class="certificate-date">Date: ${currentDate}</div>
        
        <div class="patient-info">
          <strong>Patient Information:</strong><br>
          Name: ${patient.first_name} ${patient.last_name}<br>
          Date of Birth: ${new Date(patient.date_of_birth).toLocaleDateString()}<br>
          Gender: ${patient.gender}<br>
          ID: ${patient.id_number || patient.id.substring(0, 8)}
        </div>
        
        <div class="certificate-body">
          <p>This is to certify that ${patient.first_name} ${patient.last_name} has been examined by me on ${currentDate} and is diagnosed with ${watch('diagnosis')}.</p>
          
          <p>Due to the above condition, the patient is advised to rest from <strong>${startDate}</strong> to <strong>${endDate}</strong> (${daysOfRest} day${daysOfRest !== 1 ? 's' : ''}).</p>
          
          <p><strong>Reason:</strong> ${medicalCertificateDetails.reason || 'Medical condition requiring rest and recovery'}</p>
          
          <p><strong>Recommendations:</strong> ${medicalCertificateDetails.recommendations || 'Rest and follow prescribed treatment'}</p>
        </div>
        
        <div class="signature-area">
          <div class="signature">
            <div>Dr. ${user?.first_name || ''} ${user?.last_name || ''}</div>
            <div>${user?.specialization || 'Physician'}</div>
            <div>License #: _______________</div>
          </div>
        ...
        <div class="footer">
          ...
          <p>Certificate No: MC-${Date.now().toString().substring(6)}</p>
        </div>
        
        <div class="no-print" style="margin-top: 30px; text-align: center;">
          <button onclick="window.print()" style="padding: 10px 20px; background-color: #00afaf; color: white; border: none; border-radius: 4px; cursor: pointer;">
            Print Certificate
          </button>
        </div>
      </body>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button 
                  type="button" 
                  onClick={() => toggleCollapse('patientHistory')}
                  className="text-gray-500 hover:text-gray-700"
                >
                  {collapsedSections.patientHistory ? <ChevronDown className="h-5 w-5" /> : <ChevronUp className="h-5 w-5" />}
                </button>
              </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button 
                  type="button" 
                  onClick={() => toggleCollapse('familyHistory')}
                  className="text-gray-500 hover:text-gray-700"
                >
                  {collapsedSections.familyHistory ? <ChevronDown className="h-5 w-5" /> : <ChevronUp className="h-5 w-5" />}
                </button>
              </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button 
                  type="button" 
                  onClick={() => toggleCollapse('generalExamination')}
                  className="text-gray-500 hover:text-gray-700"
                >
                  {collapsedSections.generalExamination ? <ChevronDown className="h-5 w-5" /> : <ChevronUp className="h-5 w-5" />}
                </button>
              </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button 
                  type="button" 
                  onClick={() => toggleCollapse('systemicExamination')}
                  className="text-gray-500 hover:text-gray-700"
                >
                  {collapsedSections.systemicExamination ? <ChevronDown className="h-5 w-5" /> : <ChevronUp className="h-5 w-5" />}
                </button>
              </div>
                    ...
                    <div className="relative">
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          value={selectedMedicationIndex === index ? medicationSearchTerm : prescriptions[index]?.medication || ''}
                          onChange={(e) => {
                          ...
                          onFocus={() => {
                            setSelectedMedicationIndex(index);
                            setMedicationSearchTerm(prescriptions[index]?.medication || '');
                          }}
                        />
                        
                        {selectedMedicationIndex === index && showMedicationDropdown && (
                          <div className="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base overflow-auto focus:outline-none sm:text-sm">
                            {medicationSearchResults.map((medication, i) => (
                              <div
                                key={i}
                                className="cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-gray-100"
                                onClick={() => handleMedicationSelect(medication, index)}
                              >
                                <span className="block truncate">{medication}</span>
                              </div>
                            ))}
                            <div 
                              className="cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-gray-100 border-t border-gray-200 text-primary-600"
                              onClick={() => {
                                setShowAddCustom(true);
                                setCustomMedication(medicationSearchTerm);
                              }}
                            >
      ...
      {showAddCustom && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <h3 className="text-lg font-medium text-gray-900 mb-4">Add Custom Medication</h3>
