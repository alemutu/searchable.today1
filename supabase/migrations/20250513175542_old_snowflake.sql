...
BEGIN
  ...
  -- Create departments for the hospital (only if they don't exist)
  FOR i IN 1..array_length(v_department_names, 1) LOOP
    -- Check if department already exists
    IF NOT EXISTS (
      SELECT 1 FROM departments 
      WHERE hospital_id = v_hospital_id AND name = v_department_names[i]
    ) THEN
  ...
  -- Check if license already exists for this hospital
  IF NOT EXISTS (
    SELECT 1 FROM licenses WHERE hospital_id = v_hospital_id
  ) THEN
