/*
  ...
  1. New Records
    - Creates a new hospital record with sample data
    - Creates departments for the hospital
    - Creates a pricing plan if none exists
    - Creates a license for the hospital
    - Creates hospital modules
    - Creates billing and clinical settings

  This migration creates a complete hospital setup without creating user profiles
  that would require auth.users entries.
*/
...
-- Note: We've removed the admin user creation part since we can't create auth users in migrations
-- Admin users should be created through the application interface or a separate script